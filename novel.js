...
function createLock(directory, sendUpdate) {
  ...
  if (fs.existsSync(lockFilePath)) {
    sendUpdate(
      `Another process is already running for ${directory}. Cannot start a new one.`
    );
    return false;
  }
    ...
    while (attempts < maxAttempts) {
      try {
        ...
        if (url.includes("royalroad")) {
          const chapterTitle = $(
            "body > div.page-container > div > div > div > div > div.row.fic-header.margin-bottom-40 > div > div.col-md-5.col-lg-6.col-md-offset-1.text-center.md-text-left > h1"
          )
            .text()
            .trim();
          const chapterContainer = $(".chapter-content");
          const nextChapterLink = $(
            "div.portlet-body > div.row.nav-buttons > div.col-xs-6.col-md-4.col-md-offset-4.col-lg-3.col-lg-offset-6 > a"
          ).attr("href");

          // Log the chapter title and next chapter link
          sendUpdate(
            `Fetched title: "${chapterTitle}", Looking for next chapter link...`
          );
          ...
          if (nextChapterLink) {
            ...
            sendUpdate(
              `Successfully fetched chapter "${chapterTitle}" and next: "${nextChapterUrl}"`
            );
            ...
            const chapterText = `<h1>${chapterTitle}</h1>\n${chapterContainer.html()}`;
            sendUpdate(
              `Successfully fetched chapter: "${chapterTitle}" without a next link.`
            );
          ...
          const chapterTitle =
            $(".chapter-title").text().trim() || "Untitled Chapter";
          ...
          const nextChapterUrl = nextChapterLink.length
            ? new URL(nextChapterLink, url).toString()
            : null;
        ...
        if (error.response && error.response.status === 429) {
          attempts++;
          sendUpdate(
            `Received 429 error! Attempt ${attempts} of ${maxAttempts}. Retrying in 15 seconds...`
          );
          await new Promise((resolve) => setTimeout(resolve, 15000)); // Wait for 15 seconds before retrying
        } else {
          sendUpdate(
            `Failed to fetch chapter content from ${url}: ${error.message}`
          ); // Log error message for other errors
          return [null, null];
        }
      }
    }
...
async function createEpub(title, author, directory, sendUpdate) {
  ...
  try {
    ...
    sendUpdate(
      `ePub created successfully: <a href="/${title}/${title}.epub" download>Download Here</a>`
    );
  } catch (err) {
...
function extractChapterNumber(chapterText, directory) {
  ...
  const existingChapters = files.filter(
    (file) => file.startsWith("chapter_") && file.endsWith(".html")
  ).length;
...
async function downloadChapters(
  ...
  if (existingChapters.length > 0) {
    ...
    const lastChapterContent = fs.readFileSync(
      path.join(dirPath, lastChapterFile),
      "utf-8"
    );
    ...
    if (lastChapterTitle === currentChapterTitle) {
      sendUpdate(
        `The chapter from startUrl "${url}" matches the last downloaded chapter. Skipping this chapter.`
      );
      url = nextUrl; // Move to the next chapter URL
    } else {
      sendUpdate(
        `The chapter from startUrl "${url}" differs from the last downloaded chapter. Restarting from this chapter.`
      );
...
async function getTitlePage(url, sendUpdate) {
  if (url.includes("royalroad")) {
    try {
      ...
      const $ = cheerio.load(response.data);
      const title = $(
        "div.page-content-inner > div > div.row.fic-header > div.col-md-5.col-lg-6.text-center.md-text-left.fic-title > div > h1"
      )
        .text()
        .trim();
      const author = $(
        "div.page-content-inner > div > div.row.fic-header > div.col-md-5.col-lg-6.text-center.md-text-left.fic-title > div > h4 > span:nth-child(2)"
      )
        .text()
        .trim();
      ...
      const status = $(
        "div.page-content-inner > div > div.fiction.row > div > div.fiction-info > div.portlet.light.row > div.col-md-8 > div.margin-bottom-10 > span:nth-child(2)"
      )
        .text()
        .trim();
      if (status == "STUB") {
        sendUpdate(
          "This book seems to have chapters missing due to the author removing them...this could mean that they removed them in adherance to 3rd party requirements."
        );
        return [null, null, null, null, null, null, null];
      }
      const startUrl = new URL(
        $(
          "div.page-content-inner > div > div.row.fic-header > div.col-md-4.col-lg-3.fic-buttons.text-center.md-text-left > a"
        ).attr("href"),
        rootUrl
      ).toString();
      const coverUrl = $(
        "body > div.page-container > div > div > div > div.page-content-inner > div > div.row.fic-header > div.col-md-3.text-center.cover-col > div > img"
      ).attr("src");
      const chapterCount = $(
        "div.page-content-inner > div > div.fiction.row > div > div.fiction-info > div:nth-child(5) > div.portlet-title > div.actions > span"
      )
        .text()
        .replace(" Chapters", "")
        .trim();
    ...
    try {
      ...
      const startUrl = new URL(
        $("div.novel-info > nav > a:nth-child(1)").attr("href"),
        rootUrl
      ).toString();
      console.log(startUrl);
      const coverUrl = $("figure.cover img").attr("data-src");
      const chapterCount = $("div.header-stats span strong")
        .text()
        .trim()
        .split(" ")[0];
      const status = $("div.header-stats span:last-child")
        .text()
        .replace("Status", "")
        .trim();
...
app.get("/cron", (req, res) => {
  ...
  // loading from a different path, make sure to update the file path
  const booksPath = path.join(__dirname, "books.json");
  fs.readFile(booksPath, "utf8", (err, data) => {
    if (err) {
      console.error(err);
    ...
    books.forEach((book) => {
      ...
      if (book.status.toLowerCase() == "ongoing") {
        ...
        try {
          ...
          getTitlePage(book.url, sendUpdate).then(
            ([
              title,
              author,
              startUrl,
...
app.post("/download", (req, res) => {
  ...
  if (client) {
    ...
    getTitlePage(startUrl, sendUpdate).then(
      ([title, author, ch1, coverUrl, totalChapters, status, url]) => {
        if (title) {
          ...
          try {
            ...
            const existingChapters = existingFiles.filter(
              (file) => file.startsWith("chapter_") && file.endsWith(".html")
            ).length;
            ...
            if (existingChapters >= totalChapters) {
              ...
              if (
                !fs.existsSync(path.join("public", directory, `${title}.epub`))
              ) {
                // Confirm if cover exists, then compile epub
                if (
                  !fs.existsSync(path.join("public", directory, "cover.jpg"))
                ) {
                  downloadCoverImage(coverUrl, directory, sendUpdate)
                    .then(() =>
                      createEpub(title, author, directory, sendUpdate)
                    )
                    .then(() => {
                      sendUpdate(
                        `Up to date. Download here: <a href="/public/${directory}/${directory}.epub">${title}</a>`
                      );
                    });
                } else {
